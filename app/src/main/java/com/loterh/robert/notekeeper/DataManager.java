...
public class DataManager {
    ...
    private List<TopicInfo> mCourses = new ArrayList<>();
    ...
    public TopicInfo getCourse(String id) {
        for (TopicInfo course : mCourses) {
    ...
    public List<NoteInfo> getNotes(TopicInfo course) {
        ArrayList<NoteInfo> notes = new ArrayList<>();
        for (NoteInfo note : mNotes) {
            if (course.equals(note.getTopic()))
                notes.add(note);
        }
        return notes;
    }

    public int getNoteCount(TopicInfo course) {
        int count = 0;
        for (NoteInfo note : mNotes) {
            if (course.equals(note.getTopic()))
                count++;
    ...
    public void initializeExampleNotes() {
        ...
        TopicInfo course = dm.getCourse("android_intents");
    ...
    private TopicInfo initializeCourse1() {
        ...
        return new TopicInfo("android_intents", "Android Programming with Intents", modules);
    }

    private TopicInfo initializeCourse2() {
        List<ModuleInfo> modules = new ArrayList<>();
        ...
        return new TopicInfo("android_async", "Android Async Programming and Services", modules);
    }

    private TopicInfo initializeCourse3() {
        List<ModuleInfo> modules = new ArrayList<>();
        ...
        return new TopicInfo("java_lang", "Java Fundamentals: The Java Language", modules);
    }

    private TopicInfo initializeCourse4() {
        List<ModuleInfo> modules = new ArrayList<>();
        ...
        return new TopicInfo("java_core", "Java Fundamentals: The Core Platform", modules);
    }
    //endregion
